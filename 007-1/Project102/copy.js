...
async function renderTable() {
  ...
  try {
    ...
    const allWashes = await Promise.all(
      allWashesRaw.map(async (wash) => {
        ...
        if (typeof wash.status === "string") {
          ...
          // 🔄 ถ้าสถานะเปลี่ยน → อัปเดต Firestore
    ...
    // 3️⃣ กรองข้อมูล null และตามคำค้น/สถานะ
    ...
    if (pageData.length === 0) {
      ...
      pageData.forEach((wash) => {
        ...
        if (statusLower === "waiting to send" || statusLower.startsWith("waiting rewash")) {
          ...
          actionButtonsHTML = `${!esdTested ? esdButton : ""} ${shiftDateButtons}`;
...
async function updateSummary() {
  try {
    ...
    const washes = await getAll(COLLECTIONS.WASHES);     // ข้อมูล wash ปัจจุบัน
    const history = await getAll("washHistory");         // ข้อมูลในประวัติ

    // 📌 สรุปค่าต่าง ๆ
    const total = washes.length;                                      // จำนวนทั้งหมด
    const waiting = washes.filter((w) => w.status.includes("Waiting")).length;   // กำลังรอ
    const washing = washes.filter((w) => w.status === "Washing").length;         // กำลังซัก
    const completed = washes.filter((w) => w.status === "Completed").length;     // ซักเสร็จ
    const rewash = washes.filter((w) => w.status.includes("Rewash")).length;     // ซักซ้ำ
    const scrap = washes.filter((w) => w.status === "Scrap").length;             // ทิ้ง
    const historyCount = history.length;                                         // จำนวนในประวัติ
...
// 📘 ฟังก์ชันสำหรับ auto-fill dropdown สีของ Uniform โดยค้นจากรหัส
async function autofillUniformInfo(event) {
  // 📥 รับค่าที่ผู้ใช้กรอกจากช่อง input
  const code = document.getElementById("uniformCode").value.trim();
  const colorSelect = document.getElementById("color"); // dropdown สี

  // ⛔ ออกจากฟังก์ชันทันที ถ้าไม่ได้กดปุ่ม Enter
  if (event.key !== "Enter") return;

  // ⚠️ แจ้งเตือนถ้าไม่กรอกรหัส
  if (!code) {
  ...
  try {
    // 🔍 Query หา document จาก collection 'uniformCodes' ที่มี uniformCode ตรงกับที่ผู้ใช้กรอก
    ...
    // ❌ ถ้าไม่เจอชุดที่ตรงกับรหัสที่กรอก
    if (querySnapshot.empty) {
      alert("❌ ไม่พบรหัส Uniform นี้ในระบบ");

      // ⚙️ แสดงข้อความว่าไม่มีสีให้เลือก และปิดการใช้งาน dropdown สี
      colorSelect.innerHTML = '<option value="">No Color Available</option>';
      colorSelect.disabled = true;

      // 🔄 เคลียร์ข้อมูลที่กรอกไปก่อนหน้า
      clearForm();
      return;
    }

    // ✅ ดึงเฉพาะค่าของ field "color" จาก document ที่เจอ
    const colors = querySnapshot.docs.map((doc) => doc.data().color);

    // 🧠 ลบสีที่ซ้ำกันออกด้วย Set
    const uniqueColors = [...new Set(colors)];

    // 🔄 ล้าง dropdown และใส่ option ใหม่ทั้งหมด
    ...
    // 🔓 เปิด dropdown ให้สามารถเลือกได้
    colorSelect.disabled = false;

    // 🧹 รีเซ็ตค่า employee และ quantity เพื่อให้เลือกใหม่
    document.getElementById("empId").value = "";
    document.getElementById("empName").value = "";
    document.getElementById("qty").value = 1;

    // 👉 โฟกัสไปที่ dropdown สี
    colorSelect.focus();
  } catch (err) {
...
// ⌨️ เพิ่ม event listener เมื่อผู้ใช้กดปุ่มใน input รหัสยูนิฟอร์ม
document
  .getElementById("uniformCode")
...
async function saveWash() {
  ...
  // ⚠️ ตรวจสอบความครบถ้วนก่อนบันทึก
  ...
  const washId = `WASH-${Date.now()}`; // 🔐 รหัสซักแบบ timestamp
  ...
  try {
    ...
    // 🔍 ตรวจสอบว่ายูนิฟอร์มนี้อยู่ในสถานะ scrap หรือไม่ / หรือถูกซักซ้ำครบ 3 ครั้ง
    const scrapCheckQuery = query(
      collection(db, COLLECTIONS.UNIFORMCODES),
      where("uniformCode", "==", uniformCode),
      where("color", "==", color)
    );
    const scrapSnap = await getDocs(scrapCheckQuery);
    if (!scrapSnap.empty) {
      const uniform = scrapSnap.docs[0].data();

      // ✅ ถ้าสถานะ scrap → ห้ามใช้งาน
      if (uniform.status === "scrap" || uniform.usageStatus === "scrap") {
        alert("❌ ยูนิฟอร์มนี้ถูก Scrap แล้ว ไม่สามารถใช้งานได้");
        return;
      }

      // ✅ ถ้าเคยซักซ้ำครบ 3 ครั้งแล้ว → ห้ามใช้งาน
      const rewashCount = uniform.rewashCount || 0;
      if (rewashCount >= 3) {
        alert("❌ ยูนิฟอร์มนี้ถูกซักซ้ำครบ 3 ครั้งแล้ว และจะถูก Scrap ไม่สามารถใช้งานได้");
        return;
      }
    }

    // 🧼 ค่าเริ่มต้นของการซัก (เริ่มต้นซักรอบแรก)
    let rewashCount = 0;
    ...
    const newWash = {
      ...
      color,
      rewashCount,
      status: "Waiting to Send", // 🟡 เริ่มต้นรอซัก
    };

    // 📝 บันทึกเข้า Firestore
    await setDoc(doc(db, COLLECTIONS.WASHES, washId), newWash);

    // 🔄 อัปเดตสถานะในตาราง uniform ว่าอยู่ระหว่างซัก
    ...
    if (!uniformQuerySnapshot.empty) {
      ...
      await updateDoc(doc(db, COLLECTIONS.UNIFORMCODES, uniformDocId), {
    ...
    toggleModal(false); // ปิดฟอร์ม
    renderTable();      // โหลดข้อมูลใหม่
    updateSummary();    // อัปเดตสรุป
  } catch (error) {
