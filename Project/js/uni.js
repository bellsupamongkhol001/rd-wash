...
const uniformCol = collection(db, "uniforms");
...
window.openUniformForm = async function (id = null) {
  ...
  if (id) {
    ...
    const uniDoc = await getDoc(doc(db, "uniforms", id));
    if (uniDoc.exists()) {
...
window.saveUniform = async function (e) {
  ...
  const saveToFirebase = async (photo) => {
    await setDoc(doc(db, "uniforms", id), {
      uniformId: id,
      uniformName: name,
      size,
      color,
...
window.deleteUniform = async function () {
  ...
  await deleteDoc(doc(db, "uniforms", currentDeleteId));
