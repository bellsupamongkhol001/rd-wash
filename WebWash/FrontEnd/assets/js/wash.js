...
async function saveWash() {
  ...
  try {
    ...
    if (!dupSnap.empty) {
      alert("⚠️ พนักงานนี้มียูนิฟอร์มเดียวกันอยู่แล้วในระบบ");
      return;
    }

    // 🔍 ตรวจสอบ scrap และ rewash
    ...
    if (uniSnap.empty) {
      alert("❌ ไม่พบยูนิฟอร์มในระบบ");
      return;
    }

    const uniformDoc = uniSnap.docs[0];
    const uniformData = uniformDoc.data();

    if (
      uniformData.status === "scrap" ||
      uniformData.usageStatus === "scrap" ||
      (uniformData.rewashCount || 0) >= 3
    ) {
      alert("❌ ยูนิฟอร์มนี้ใช้งานไม่ได้ (Scrap หรือซักซ้ำครบ 3 ครั้ง)");
      return;
    }

    // ✅ คำนวณสถานะตามเวลาปัจจุบัน (Real-time)
    const now = new Date();
    const created = new Date(createdAt);
    const diffDays = Math.floor((now - created) / (1000 * 60 * 60 * 24));
    let realtimeStatus = "Waiting to Send";
    if (diffDays >= 3) realtimeStatus = "Completed";
    else if (diffDays >= 1) realtimeStatus = "Washing";
    ...
    const newWash = {
      ...
      createdAt,
      rewashCount: 0,
      status: realtimeStatus // ✅ อิงตามเวลาจริง
    };

    // ✅ บันทึกข้อมูลซัก
    await setDoc(doc(db, COLLECTIONS.WASHES, washId), newWash);

    // ✅ อัปเดต Inventory ด้วยสถานะ real-time เช่นเดียวกัน
    const uniformRef = doc(db, COLLECTIONS.UNIFORMCODES, uniformDoc.id);
    await updateDoc(uniformRef, {
      status: "in-use/washing",          // ✅ สถานะการใช้งาน
      washStatus: realtimeStatus,        // ✅ แสดงสถานะ real-time ซัก
      lastWash: createdAt,
    ...
    await renderTable();
    await updateSummary();
    showNotificationModal("✅ บันทึกข้อมูลซักสำเร็จ พร้อมแสดงสถานะตามเวลา");
  } catch (err) {
